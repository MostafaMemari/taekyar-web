...
export function normalizeTaxonomyInput(input: TaxonomyInput): TaxonomyValidationResult {
  ...
  return {
    ...
    data: {
      ...
      description: description ? sanitizePostHtml(description) : null,
...
export type PostValidationResult =
  ...
  | { ok: false; fieldErrors: PostFieldErrors };export function normalizePostInput(input: PostInput): PostValidationResult {
...
export type PageValidationResult =
  | { ok: true; data: NormalizedPage; slugAutoGenerated: boolean }
  | { ok: false; fieldErrors: PageFieldErrors };

export function normalizePageInput(input: PageInput): PageValidationResult {
  const errors: PageFieldErrors = {};

  const title = String(input.title ?? "").trim();
  if (!title) errors.title = PAGES_FORM_LABELS.titleRequired;

  const slugResolution = resolveSlug(String(input.slug ?? ""), title);
  if (!slugResolution.ok) {
    errors.slug =
      slugResolution.reason === "empty" ? PAGES_FORM_LABELS.slugRequired : PAGES_FORM_LABELS.slugInvalid;
  }
